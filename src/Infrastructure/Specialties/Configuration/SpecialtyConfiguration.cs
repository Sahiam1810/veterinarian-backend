...
public sealed class SpecialtyConfiguration : IEntityTypeConfiguration<SpecialtyEntity>
{
    public void Configure(EntityTypeBuilder<SpecialtyEntity> builder)
    {
        ...
        builder.Property(x => x.Name).HasColumnName("NAME").HasColumnType("VARCHAR2(30)").HasMaxLength(SpecialtyName.MaxLength)
            .HasConversion(name => name.Value, value => SpecialtyName.Create(value)).IsRequired();
        builder.Property(x => x.Description).HasColumnName("DESCRIPTION").HasColumnType("VARCHAR2(30)").HasMaxLength(SpecialtyDescription.MaxLength)
