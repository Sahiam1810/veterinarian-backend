...
public sealed class SpecialtyConfiguration : IEntityTypeConfiguration<SpecialtyEntity>
{
    public void Configure(EntityTypeBuilder<SpecialtyEntity> builder)
    {
        ...
        builder.Property(x => x.Id).HasColumnName("SPECIALTY_ID").HasColumnType("VARCHAR2(36)")
            .HasConversion(guid => guid.ToString(), value => Guid.Parse(value)).IsRequired().ValueGeneratedNever();
        builder.Property(x => x.Name).HasColumnName("NAME").HasColumnType("VARCHAR2(120)").HasMaxLength(SpecialtyName.MaxLength)
            .HasConversion(name => name.Value, value => SpecialtyName.Create(value)).IsRequired();
        builder.Property(x => x.Description).HasColumnName("DESCRIPTION").HasColumnType("VARCHAR2(120)").HasMaxLength(SpecialtyDescription.MaxLength)
            .HasConversion(description => description.Value, value => SpecialtyDescription.Create(value)).IsRequired(false);
