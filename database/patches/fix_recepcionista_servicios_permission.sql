...
-- (Recepcionista, Servicios) quedo con CAN_VIEW=0 en bases ya sembradas,
...
-- con un valor distinto: se requiere este patch idempotente. Mismo patron
-- ya corregido para (Recepcionista, Veterinarios) en
-- fix_recepcionista_veterinarios_permission.sql.
--
-- Efecto observado: GET /api/Services (y /api/TypeServices, ambos gateados
-- por RequirePermission("Servicios", View)) devuelve 403 para Recepcionista,
-- por lo que el selector "Motivo de Consulta" del modulo de Agenda y Citas
-- solo muestra un servicio de respaldo ("Consulta General") en vez de los
-- servicios reales creados desde el panel de SuperAdmin.
SET DEFINE OFF
SET SERVEROUTPUT ON
...
BEGIN
  UPDATE ROLE_PERMISSIONS
   ...
   WHERE ROLE_ID = (SELECT ROLE_ID FROM ROLES WHERE NAME = 'Recepcionista')
     AND MODULE_ID = (SELECT MODULE_ID FROM MODULES WHERE NAME = 'Servicios')
     AND CAN_VIEW = 0;
  v_rows := SQL%ROWCOUNT;
  IF v_rows > 0 THEN
...
SELECT r.NAME AS ROLE_NAME, m.NAME AS MODULE_NAME, rp.CAN_VIEW
 ...
 WHERE r.NAME = 'Recepcionista' AND m.NAME = 'Servicios';
EXIT
