...
-- Hash generado con la implementación real de IPasswordHasher de la app
-- (Infrastructure.Security.PasswordHasher: PBKDF2-HMAC-SHA256, 100000
-- iteraciones, salt de 16 bytes, clave de 32 bytes; formato de 3 partes
-- separadas por '.': iterations.base64(salt).base64(key)). NO es un hash
-- de bcrypt/otra librería: fue calculado invocando esa misma clase.
...
USING (
    ...
    SELECT 'bbbb0004-0000-0000-0000-000000000001' AS ID, 'Admin General' AS FULL_NAME, 'admin@veterinaria.com' AS EMAIL,
           '100000.uDRzSo2R4QKRRm5lFo9i4w==.j6rbqW/M2w062GPACpVAwwn4cBEni67eUHwlh8VIbN4=' AS PASSWORD_HASH,
    ...
    -- Dominio '@clientes.test' (TLD .test, reservado por IANA/RFC 2606 y
    -- nunca delegable) para que estos correos ficticios de prueba jamás
    -- colisionen con una cuenta real registrada en la app (p.ej. un
    -- gmail.com genérico sí podría coincidir con un usuario real existente).
    SELECT 'bbbb0004-0000-0000-0000-000000000005' AS ID, 'Ana Gomez' AS FULL_NAME, 'ana.gomez@clientes.test' AS EMAIL,
    ...
    SELECT 'bbbb0004-0000-0000-0000-000000000006' AS ID, 'Roberto Carlos' AS FULL_NAME, 'roberto.carlos@clientes.test' AS EMAIL,
           CAST(NULL AS VARCHAR2(255)) AS PASSWORD_HASH,
           '77777777-7777-7777-7777-777777777777' AS ROLE_ID, 1 AS IS_ACTIVE FROM DUAL UNION ALL
    SELECT 'bbbb0004-0000-0000-0000-000000000007' AS ID, 'Sofia Vergara' AS FULL_NAME, 'sofia.vergara@clientes.test' AS EMAIL,
           CAST(NULL AS VARCHAR2(255)) AS PASSWORD_HASH,
           '77777777-7777-7777-7777-777777777777' AS ROLE_ID, 1 AS IS_ACTIVE FROM DUAL UNION ALL
    SELECT 'bbbb0004-0000-0000-0000-000000000008' AS ID, 'Luis Miguel' AS FULL_NAME, 'luis.miguel@clientes.test' AS EMAIL,
           CAST(NULL AS VARCHAR2(255)) AS PASSWORD_HASH,
           '77777777-7777-7777-7777-777777777777' AS ROLE_ID, 1 AS IS_ACTIVE FROM DUAL UNION ALL
    SELECT 'bbbb0004-0000-0000-0000-000000000009' AS ID, 'Shakira Mebarak' AS FULL_NAME, 'shakira.mebarak@clientes.test' AS EMAIL,
           CAST(NULL AS VARCHAR2(255)) AS PASSWORD_HASH,
           '77777777-7777-7777-7777-777777777777' AS ROLE_ID, 1 AS IS_ACTIVE FROM DUAL UNION ALL
    SELECT 'bbbb0004-0000-0000-0000-000000000010' AS ID, 'Lionel Messi' AS FULL_NAME, 'lionel.messi@clientes.test' AS EMAIL,
           CAST(NULL AS VARCHAR2(255)) AS PASSWORD_HASH,
...
-- =============================================================================
-- 5. USER_ACCOUNTS (Cuentas de usuario para login por email)
...
-- STATUS debe ser exactamente 'Activo' (literal usado por
-- AuthenticationService.IsActiveAccount / AccountStatus.Active); cualquier
-- otro valor (p.ej. 'Active') hace que el login falle con InvalidCredentials
-- antes de siquiera verificar la contraseña.
-- =============================================================================
...
USING (
    ...
    SELECT 'bbbb0013-0000-0000-0000-000000000002' AS ID, 'bbbb0008-0000-0000-0000-000000000002' AS CLIENT_PET_ID, 'bbbb0005-0000-0000-0000-000000000001' AS VET_ID, 'bbbb0010-0000-0000-0000-000000000001' AS SERVICE_ID, (SELECT STATUS_APPOINTMENT_ID FROM STATUS_APPOINTMENTS WHERE UPPER(NAME) = 'ATENDIDA' AND ROWNUM = 1) AS STATUS_ID, 'bbbb0012-0000-0000-0000-000000000001' AS AVAILABILITY_ID, TO_TIMESTAMP('2026-09-02 10:00:00', 'YYYY-MM-DD HH24:MI:SS') AS SCHED_START, TO_TIMESTAMP('2026-09-02 10:30:00', 'YYYY-MM-DD HH24:MI:SS') AS SCHED_END, 'Consulta control' AS NOTES, '3209876543' AS PHONE FROM DUAL UNION ALL
    SELECT 'bbbb0013-0000-0000-0000-000000000003' AS ID, 'bbbb0008-0000-0000-0000-000000000003' AS CLIENT_PET_ID, 'bbbb0005-0000-0000-0000-000000000001' AS VET_ID, 'bbbb0010-0000-0000-0000-000000000001' AS SERVICE_ID, (SELECT STATUS_APPOINTMENT_ID FROM STATUS_APPOINTMENTS WHERE UPPER(NAME) = 'AGENDADA' AND ROWNUM = 1) AS STATUS_ID, 'bbbb0012-0000-0000-0000-000000000001' AS AVAILABILITY_ID, TO_TIMESTAMP('2026-10-01 11:00:00', 'YYYY-MM-DD HH24:MI:SS') AS SCHED_START, TO_TIMESTAMP('2026-10-01 11:30:00', 'YYYY-MM-DD HH24:MI:SS') AS SCHED_END, 'Revisión general' AS NOTES, '3001112233' AS PHONE FROM DUAL UNION ALL
    SELECT 'bbbb0013-0000-0000-0000-000000000004' AS ID, 'bbbb0008-0000-0000-0000-000000000004' AS CLIENT_PET_ID, 'bbbb0005-0000-0000-0000-000000000001' AS VET_ID, 'bbbb0010-0000-0000-0000-000000000002' AS SERVICE_ID, (SELECT STATUS_APPOINTMENT_ID FROM STATUS_APPOINTMENTS WHERE UPPER(NAME) = 'ATENDIDA' AND ROWNUM = 1) AS STATUS_ID, 'bbbb0012-0000-0000-0000-000000000002' AS AVAILABILITY_ID, TO_TIMESTAMP('2026-09-03 09:00:00', 'YYYY-MM-DD HH24:MI:SS') AS SCHED_START, TO_TIMESTAMP('2026-09-03 09:30:00', 'YYYY-MM-DD HH24:MI:SS') AS SCHED_END, 'Vacunación' AS NOTES, '3115556677' AS PHONE FROM DUAL UNION ALL
    SELECT 'bbbb0013-0000-0000-0000-000000000005' AS ID, 'bbbb0008-0000-0000-0000-000000000005' AS CLIENT_PET_ID, 'bbbb0005-0000-0000-0000-000000000001' AS VET_ID, 'bbbb0010-0000-0000-0000-000000000001' AS SERVICE_ID, (SELECT STATUS_APPOINTMENT_ID FROM STATUS_APPOINTMENTS WHERE UPPER(NAME) = 'ATENDIDA' AND ROWNUM = 1) AS STATUS_ID, 'bbbb0012-0000-0000-0000-000000000002' AS AVAILABILITY_ID, TO_TIMESTAMP('2026-09-04 10:00:00', 'YYYY-MM-DD HH24:MI:SS') AS SCHED_START, TO_TIMESTAMP('2026-09-04 10:30:00', 'YYYY-MM-DD HH24:MI:SS') AS SCHED_END, 'Problema estomacal' AS NOTES, '3228889900' AS PHONE FROM DUAL UNION ALL
    SELECT 'bbbb0013-0000-0000-0000-000000000006' AS ID, 'bbbb0008-0000-0000-0000-000000000006' AS CLIENT_PET_ID, 'bbbb0005-0000-0000-0000-000000000001' AS VET_ID, 'bbbb0010-0000-0000-0000-000000000002' AS SERVICE_ID, (SELECT STATUS_APPOINTMENT_ID FROM STATUS_APPOINTMENTS WHERE UPPER(NAME) = 'ATENDIDA' AND ROWNUM = 1) AS STATUS_ID, 'bbbb0012-0000-0000-0000-000000000002' AS AVAILABILITY_ID, TO_TIMESTAMP('2026-09-05 11:00:00', 'YYYY-MM-DD HH24:MI:SS') AS SCHED_START, TO_TIMESTAMP('2026-09-05 11:30:00', 'YYYY-MM-DD HH24:MI:SS') AS SCHED_END, 'Vacunación' AS NOTES, '3144433322' AS PHONE FROM DUAL UNION ALL
    SELECT 'bbbb0013-0000-0000-0000-000000000007' AS ID, 'bbbb0008-0000-0000-0000-000000000007' AS CLIENT_PET_ID, 'bbbb0005-0000-0000-0000-000000000001' AS VET_ID, 'bbbb0010-0000-0000-0000-000000000001' AS SERVICE_ID, (SELECT STATUS_APPOINTMENT_ID FROM STATUS_APPOINTMENTS WHERE UPPER(NAME) = 'ATENDIDA' AND ROWNUM = 1) AS STATUS_ID, 'bbbb0012-0000-0000-0000-000000000001' AS AVAILABILITY_ID, TO_TIMESTAMP('2026-09-06 12:00:00', 'YYYY-MM-DD HH24:MI:SS') AS SCHED_START, TO_TIMESTAMP('2026-09-06 12:30:00', 'YYYY-MM-DD HH24:MI:SS') AS SCHED_END, 'Control alas' AS NOTES, '3144433322' AS PHONE FROM DUAL
) source
...
USING (
    ...
    SELECT 'bbbb0014-0000-0000-0000-000000000002' AS ID, 'bbbb0008-0000-0000-0000-000000000002' AS CLIENT_PET_ID, 'bbbb0013-0000-0000-0000-000000000002' AS APPOINTMENT_ID, 'bbbb0011-0000-0000-0000-000000000002' AS DIAGNOSTIC_ID, 'Control sano' AS SYMPTOMS, 'Ninguno' AS TREATMENT, 4.3 AS WEIGHT_AT_VISIT, 38.0 AS TEMPERATURE FROM DUAL UNION ALL
    SELECT 'bbbb0014-0000-0000-0000-000000000003' AS ID, 'bbbb0008-0000-0000-0000-000000000004' AS CLIENT_PET_ID, 'bbbb0013-0000-0000-0000-000000000004' AS APPOINTMENT_ID, 'bbbb0011-0000-0000-0000-000000000002' AS DIAGNOSTIC_ID, 'Requiere vacuna' AS SYMPTOMS, 'Aplicación de vacuna' AS TREATMENT, 3.6 AS WEIGHT_AT_VISIT, 38.2 AS TEMPERATURE FROM DUAL UNION ALL
    SELECT 'bbbb0014-0000-0000-0000-000000000004' AS ID, 'bbbb0008-0000-0000-0000-000000000005' AS CLIENT_PET_ID, 'bbbb0013-0000-0000-0000-000000000005' AS APPOINTMENT_ID, 'bbbb0011-0000-0000-0000-000000000001' AS DIAGNOSTIC_ID, 'Vómito y diarrea' AS SYMPTOMS, 'Hidratación y dieta blanda' AS TREATMENT, 34.5 AS WEIGHT_AT_VISIT, 39.5 AS TEMPERATURE FROM DUAL UNION ALL
    SELECT 'bbbb0014-0000-0000-0000-000000000005' AS ID, 'bbbb0008-0000-0000-0000-000000000006' AS CLIENT_PET_ID, 'bbbb0013-0000-0000-0000-000000000006' AS APPOINTMENT_ID, 'bbbb0011-0000-0000-0000-000000000002' AS DIAGNOSTIC_ID, 'Control vacunas' AS SYMPTOMS, 'Aplicación vacuna múltiple' AS TREATMENT, 8.2 AS WEIGHT_AT_VISIT, 38.1 AS TEMPERATURE FROM DUAL UNION ALL
    SELECT 'bbbb0014-0000-0000-0000-000000000006' AS ID, 'bbbb0008-0000-0000-0000-000000000007' AS CLIENT_PET_ID, 'bbbb0013-0000-0000-0000-000000000007' AS APPOINTMENT_ID, 'bbbb0011-0000-0000-0000-000000000002' AS DIAGNOSTIC_ID, 'Revisión plumaje' AS SYMPTOMS, 'Vitaminas' AS TREATMENT, 0.5 AS WEIGHT_AT_VISIT, 40.0 AS TEMPERATURE FROM DUAL
) source
...
USING (
    SELECT 'bbbb0015-0000-0000-0000-000000000001' AS ID,
           ...
           TO_DATE('2027-09-01', 'YYYY-MM-DD') AS NEXT_DOSE_DATE FROM DUAL UNION ALL
    SELECT 'bbbb0015-0000-0000-0000-000000000002' AS ID, 'bbbb0008-0000-0000-0000-000000000004' AS CLIENT_PET_ID, 'bbbb0014-0000-0000-0000-000000000003' AS RECORD_ID, 'Triple Felina' AS VACCINE_NAME, 1 AS DOSE_NUMBER, TO_DATE('2026-09-03', 'YYYY-MM-DD') AS APPLICATION_DATE, TO_DATE('2027-09-03', 'YYYY-MM-DD') AS NEXT_DOSE_DATE FROM DUAL UNION ALL
    SELECT 'bbbb0015-0000-0000-0000-000000000003' AS ID, 'bbbb0008-0000-0000-0000-000000000006' AS CLIENT_PET_ID, 'bbbb0014-0000-0000-0000-000000000005' AS RECORD_ID, 'Séxtuple Canina' AS VACCINE_NAME, 2 AS DOSE_NUMBER, TO_DATE('2026-09-05', 'YYYY-MM-DD') AS APPLICATION_DATE, TO_DATE('2027-09-05', 'YYYY-MM-DD') AS NEXT_DOSE_DATE FROM DUAL
) source
