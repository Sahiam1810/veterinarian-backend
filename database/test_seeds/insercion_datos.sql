-- =============================================================================
-- SEED DE DATOS DE PRUEBA OPERACIONALES (TEST DATA)
...
--   4. USERS (Usuarios veterinarios y clientes vinculados a ROLES)
--   5. VETERINARIANS (Veterinarios con licencia y especialidad)
--   6. CLIENTS (Clientes con identificación y usuario)
--   7. PETS (Mascotas con especie y raza)
--   8. CLIENTS_PETS (Tabla intermedia de asignación dueño-mascota)
--   9. TYPE_SERVICES (Tipos de servicio)
--  10. SERVICES (Servicios médicos/estéticos con precio y duración)
--  11. DIAGNOSTICS (Diagnósticos con código único)
--  12. AVAILABILITIES (Horarios de disponibilidad semanal por veterinario)
--  13. STATUS_APPOINTMENTS (Garantiza estados de cita canónicos)
--  14. APPOINTMENTS (Citas médicas agendadas)
--  15. MEDICAL_RECORDS (Historias clínicas / consultas realizadas)
--  16. VACCINATIONS (Registro de vacunaciones aplicadas)
--
-- Uso de MERGE para garantizar idempotencia total.
-- =============================================================================
...
-- =============================================================================
-- 4. USERS (Roles canónicos: 4444...4444 Veterinario, 7777...7777 Cliente)
-- =============================================================================
MERGE INTO USERS target
USING (
    SELECT 'bbbb0004-0000-0000-0000-000000000001' AS ID, 'Dr. Carlos Mendoza' AS FULL_NAME, 'vet.carlos@veterinaria.com' AS EMAIL,
    ...
    SELECT 'bbbb0004-0000-0000-0000-000000000002' AS ID, 'Dra. Laura Morales' AS FULL_NAME, 'vet.laura@veterinaria.com' AS EMAIL,
           '$2a$11$eN7xN61c6iX2y7r2Gk4rAeK2o7k4f4W8l3x1Z9c0B2m5n8v7a1s3.' AS PASSWORD_HASH,
           '44444444-4444-4444-4444-444444444444' AS ROLE_ID, 1 AS IS_ACTIVE FROM DUAL UNION ALL
    SELECT 'bbbb0004-0000-0000-0000-000000000003' AS ID, 'Ana María Gómez' AS FULL_NAME, 'ana.gomez@gmail.com' AS EMAIL,
           '$2a$11$eN7xN61c6iX2y7r2Gk4rAeK2o7k4f4W8l3x1Z9c0B2m5n8v7a1s3.' AS PASSWORD_HASH,
           '77777777-7777-7777-7777-777777777777' AS ROLE_ID, 1 AS IS_ACTIVE FROM DUAL UNION ALL
    SELECT 'bbbb0004-0000-0000-0000-000000000004' AS ID, 'Juan David Pérez' AS FULL_NAME, 'juan.perez@hotmail.com' AS EMAIL,
           '$2a$11$eN7xN61c6iX2y7r2Gk4rAeK2o7k4f4W8l3x1Z9c0B2m5n8v7a1s3.' AS PASSWORD_HASH,
...
USING (
    SELECT 'bbbb0005-0000-0000-0000-000000000001' AS ID,
           'bbbb0004-0000-0000-0000-000000000001' AS USER_ID,
           'bbbb0001-0000-0000-0000-000000000001' AS SPECIALTY_ID,
           'MP-102938' AS LICENSE_NUMBER FROM DUAL UNION ALL
    SELECT 'bbbb0005-0000-0000-0000-000000000002' AS ID,
           'bbbb0004-0000-0000-0000-000000000002' AS USER_ID,
           'bbbb0001-0000-0000-0000-000000000002' AS SPECIALTY_ID,
           'MP-564738' AS LICENSE_NUMBER FROM DUAL
) source
...
-- =============================================================================
-- 6. CLIENTS (ON por IDENTIFICATION_NUMBER para evitar ORA-00001)
-- =============================================================================
MERGE INTO CLIENTS target
USING (
    SELECT 'bbbb0006-0000-0000-0000-000000000001' AS ID,
           'bbbb0004-0000-0000-0000-000000000003' AS USER_ID,
           '1032456789' AS IDENTIFICATION_NUMBER,
           'Calle 45 #12-34' AS ADDRESS,
           '3101234567' AS PHONE_NUMBER,
           TO_DATE('2026-01-15', 'YYYY-MM-DD') AS REGISTRATION_DATE FROM DUAL UNION ALL
    SELECT 'bbbb0006-0000-0000-0000-000000000002' AS ID,
           'bbbb0004-0000-0000-0000-000000000004' AS USER_ID,
           '1098765432' AS IDENTIFICATION_NUMBER,
           'Carrera 7 #80-15' AS ADDRESS,
           '3209876543' AS PHONE_NUMBER,
           TO_DATE('2026-02-01', 'YYYY-MM-DD') AS REGISTRATION_DATE FROM DUAL
) source
...
USING (
    ...
    SELECT 'bbbb0007-0000-0000-0000-000000000002' AS ID, 'Luna' AS NAME, 2 AS AGE, 'F' AS GENDER, 4.200 AS WEIGHT,
           ...
           'bbbb0003-0000-0000-0000-000000000004' AS RACE_ID FROM DUAL UNION ALL
    SELECT 'bbbb0007-0000-0000-0000-000000000003' AS ID, 'Rocky' AS NAME, 5 AS AGE, 'M' AS GENDER, 12.800 AS WEIGHT,
           'Tratamiento de otitis en curso' AS OBSERVATIONS,
           'bbbb0002-0000-0000-0000-000000000001' AS SPECIES_ID,
           'bbbb0003-0000-0000-0000-000000000002' AS RACE_ID FROM DUAL
) source
...
-- =============================================================================
-- 8. CLIENTS_PETS (Relación Dueño - Mascota)
-- =============================================================================
...
USING (
    ...
    SELECT 'bbbb0008-0000-0000-0000-000000000002' AS ID,
           ...
           'Y' AS IS_PRIMARY_OWNER FROM DUAL UNION ALL
    SELECT 'bbbb0008-0000-0000-0000-000000000003' AS ID,
           'bbbb0006-0000-0000-0000-000000000002' AS CLIENT_ID,
           'bbbb0007-0000-0000-0000-000000000003' AS PET_ID,
...
USING (
    ...
    SELECT 'bbbb0009-0000-0000-0000-000000000003' AS ID, 'Medicina Preventiva' AS NAME, 'Planes de vacunación y desparasitación' AS DESCRIPTION FROM DUAL UNION ALL
    SELECT 'bbbb0009-0000-0000-0000-000000000004' AS ID, 'Peluquería y Estética' AS NAME, 'Baño, corte de pelo y aseo general' AS DESCRIPTION FROM DUAL
) source
...
USING (
    ...
    SELECT 'bbbb0010-0000-0000-0000-000000000002' AS ID,
           'bbbb0009-0000-0000-0000-000000000001' AS TYPE_SERVICE_ID,
           'Consulta Especializada' AS NAME, 45 AS DURATION_MINUTES, 85000 AS PRICE, 'Y' AS IS_ACTIVE FROM DUAL UNION ALL
    SELECT 'bbbb0010-0000-0000-0000-000000000003' AS ID,
           'bbbb0009-0000-0000-0000-000000000003' AS TYPE_SERVICE_ID,
           'Aplicación de Vacuna' AS NAME, 20 AS DURATION_MINUTES, 40000 AS PRICE, 'Y' AS IS_ACTIVE FROM DUAL UNION ALL
    SELECT 'bbbb0010-0000-0000-0000-000000000004' AS ID,
           'bbbb0009-0000-0000-0000-000000000002' AS TYPE_SERVICE_ID,
           'Esterilización Canina / Felina' AS NAME, 90 AS DURATION_MINUTES, 180000 AS PRICE, 'Y' AS IS_ACTIVE FROM DUAL
) source
...
USING (
    ...
    SELECT 'bbbb0011-0000-0000-0000-000000000002' AS ID, 'DIAG-002' AS CODE, 'Dermatitis atópica' AS NAME, 'Reacción alérgica cutánea estacional o ambiental' AS DESCRIPTION, 1 AS IS_ACTIVE FROM DUAL UNION ALL
    SELECT 'bbbb0011-0000-0000-0000-000000000003' AS ID, 'DIAG-003' AS CODE, 'Otitis externa bacteriana' AS NAME, 'Infección en conducto auditivo externo' AS DESCRIPTION, 1 AS IS_ACTIVE FROM DUAL UNION ALL
    SELECT 'bbbb0011-0000-0000-0000-000000000004' AS ID, 'DIAG-004' AS CODE, 'Control sano / Preventivo' AS NAME, 'Paciente en óptimas condiciones generales' AS DESCRIPTION, 1 AS IS_ACTIVE FROM DUAL
) source
...
-- =============================================================================
-- 12. AVAILABILITIES (Disponibilidad semanal de los veterinarios)
-- =============================================================================
MERGE INTO AVAILABILITIES target
USING (
    -- Dr. Carlos Mendoza: Lunes (1), Miércoles (3), Viernes (5) de 08:00 a 14:00
    ...
    SELECT 'bbbb0012-0000-0000-0000-000000000002' AS ID, 'bbbb0005-0000-0000-0000-000000000001' AS VET_ID, 3 AS DAY_OF_WEEK, '08:00:00' AS START_TIME, '14:00:00' AS END_TIME, 'Y' AS IS_ACTIVE FROM DUAL UNION ALL
    SELECT 'bbbb0012-0000-0000-0000-000000000003' AS ID, 'bbbb0005-0000-0000-0000-000000000001' AS VET_ID, 5 AS DAY_OF_WEEK, '08:00:00' AS START_TIME, '14:00:00' AS END_TIME, 'Y' AS IS_ACTIVE FROM DUAL UNION ALL
    -- Dra. Laura Morales: Martes (2), Jueves (4), Sábado (6) de 10:00 a 16:00
    SELECT 'bbbb0012-0000-0000-0000-000000000004' AS ID, 'bbbb0005-0000-0000-0000-000000000002' AS VET_ID, 2 AS DAY_OF_WEEK, '10:00:00' AS START_TIME, '16:00:00' AS END_TIME, 'Y' AS IS_ACTIVE FROM DUAL UNION ALL
    SELECT 'bbbb0012-0000-0000-0000-000000000005' AS ID, 'bbbb0005-0000-0000-0000-000000000002' AS VET_ID, 4 AS DAY_OF_WEEK, '10:00:00' AS START_TIME, '16:00:00' AS END_TIME, 'Y' AS IS_ACTIVE FROM DUAL UNION ALL
    SELECT 'bbbb0012-0000-0000-0000-000000000006' AS ID, 'bbbb0005-0000-0000-0000-000000000002' AS VET_ID, 6 AS DAY_OF_WEEK, '09:00:00' AS START_TIME, '13:00:00' AS END_TIME, 'Y' AS IS_ACTIVE FROM DUAL
) source
...
-- =============================================================================
-- 13. STATUS_APPOINTMENTS (Garantiza los IDs canónicos esperados por APPOINTMENTS)
-- =============================================================================
...
-- =============================================================================
-- 14. APPOINTMENTS (Citas médicas con subquery dinámica a STATUS_APPOINTMENTS)
-- =============================================================================
MERGE INTO APPOINTMENTS target
USING (
    -- Cita 1: Atendida para Max con Dr. Carlos Mendoza
    SELECT 'bbbb0013-0000-0000-0000-000000000001' AS ID,
           ...
           'Chequeo general y dolor abdominal leve' AS NOTES,
           '3101234567' AS PHONE FROM DUAL UNION ALL
    -- Cita 2: Atendida para Luna (vacuna) con Dra. Laura Morales
    SELECT 'bbbb0013-0000-0000-0000-000000000002' AS ID,
           'bbbb0008-0000-0000-0000-000000000002' AS CLIENT_PET_ID,
           'bbbb0005-0000-0000-0000-000000000002' AS VET_ID,
           'bbbb0010-0000-0000-0000-000000000003' AS SERVICE_ID,
           (SELECT STATUS_APPOINTMENT_ID FROM STATUS_APPOINTMENTS WHERE UPPER(NAME) = 'ATENDIDA' AND ROWNUM = 1) AS STATUS_ID,
           'bbbb0012-0000-0000-0000-000000000004' AS AVAILABILITY_ID,
           TO_TIMESTAMP('2026-09-02 10:30:00', 'YYYY-MM-DD HH24:MI:SS') AS SCHED_START,
           TO_TIMESTAMP('2026-09-02 10:50:00', 'YYYY-MM-DD HH24:MI:SS') AS SCHED_END,
           'Vacunación anual felina' AS NOTES,
           '3101234567' AS PHONE FROM DUAL UNION ALL
    -- Cita 3: Agendada próxima para Rocky
    SELECT 'bbbb0013-0000-0000-0000-000000000003' AS ID,
           'bbbb0008-0000-0000-0000-000000000003' AS CLIENT_PET_ID,
           'bbbb0005-0000-0000-0000-000000000001' AS VET_ID,
           'bbbb0010-0000-0000-0000-000000000002' AS SERVICE_ID,
           (SELECT STATUS_APPOINTMENT_ID FROM STATUS_APPOINTMENTS WHERE UPPER(NAME) = 'AGENDADA' AND ROWNUM = 1) AS STATUS_ID,
           'bbbb0012-0000-0000-0000-000000000002' AS AVAILABILITY_ID,
           TO_TIMESTAMP('2026-09-05 11:00:00', 'YYYY-MM-DD HH24:MI:SS') AS SCHED_START,
           TO_TIMESTAMP('2026-09-05 11:45:00', 'YYYY-MM-DD HH24:MI:SS') AS SCHED_END,
           'Revisión especializada de oído' AS NOTES,
           '3209876543' AS PHONE FROM DUAL
) source
...
-- =============================================================================
-- 15. MEDICAL_RECORDS (Historias clínicas ligadas a cita y diagnóstico)
-- =============================================================================
...
USING (
    SELECT 'bbbb0014-0000-0000-0000-000000000001' AS ID,
           ...
           'bbbb0011-0000-0000-0000-000000000001' AS DIAGNOSTIC_ID,
           'Vómito leve y decaimiento' AS SYMPTOMS,
           'Probióticos y dieta blanda 5d' AS TREATMENT,
           28.4 AS WEIGHT_AT_VISIT,
           38.6 AS TEMPERATURE FROM DUAL UNION ALL
    SELECT 'bbbb0014-0000-0000-0000-000000000002' AS ID,
           'bbbb0008-0000-0000-0000-000000000002' AS CLIENT_PET_ID,
           'bbbb0013-0000-0000-0000-000000000002' AS APPOINTMENT_ID,
           'bbbb0011-0000-0000-0000-000000000004' AS DIAGNOSTIC_ID,
           'Paciente sano para vacuna' AS SYMPTOMS,
           'Vacunación triple felina' AS TREATMENT,
           4.2 AS WEIGHT_AT_VISIT,
           38.2 AS TEMPERATURE FROM DUAL
) source
...
-- =============================================================================
-- 16. VACCINATIONS (Vacunas aplicadas)
-- =============================================================================
MERGE INTO VACCINATIONS target
USING (
    SELECT 'bbbb0015-0000-0000-0000-000000000001' AS ID,
           'bbbb0008-0000-0000-0000-000000000002' AS CLIENT_PET_ID,
           'bbbb0014-0000-0000-0000-000000000002' AS RECORD_ID,
           'Triple Felina (RCP)' AS VACCINE_NAME,
           1 AS DOSE_NUMBER,
           TO_DATE('2026-09-02', 'YYYY-MM-DD') AS APPLICATION_DATE,
           TO_DATE('2027-09-02', 'YYYY-MM-DD') AS NEXT_DOSE_DATE FROM DUAL
) source
