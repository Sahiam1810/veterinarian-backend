...
### ✅ Ya no está pendiente (parcial): PII en `GET /api/clients/by-identification/{identificationNumber}`
Corregido 2026-09-03 — solo la parte que se podía resolver sin coordinación externa. El endpoint **sigue siendo anónimo a propósito** (el chatbot lo necesita para identificar clientes antes de tener JWT — confirmado contra el documento de diseño original de la feature, que describe exactamente este uso) y **sigue sin exigir ningún secreto de servicio-a-servicio** (a diferencia del webhook de Telegram, que sí valida uno) — eso queda pendiente porque requeriría coordinar un cambio del lado del chatbot (repo aparte, Huellitas_ChatBot) y no se hizo hoy para no romper la integración en producción.

Lo que sí se corrigió: la respuesta ya no expone `Address`/`PhoneNumber`. Nuevo DTO acotado `ClientIdentificationLookupResponseDto` (`Id`, `UserId`, `IdentificationNumber`, `RegistrationDate` solamente), usado exclusivamente por este endpoint — `ClientResponseDto` completo se mantiene sin cambios para `GetMe`/`GetAll`/`GetById`/`Create` (todos autenticados). Test nuevo (`ClientMappingsExtensionsTests`) fija por reflexión que esa respuesta nunca vuelva a incluir esos dos campos. `dotnet test`: 685/685.

**Pendiente real que queda:** agregar un secreto de servicio-a-servicio a este endpoint (mismo patrón que `TelegramWebhookSecretValidator`) para que solo el chatbot pueda llamarlo, no cualquiera que adivine una cédula. Necesita coordinación con el equipo del chatbot antes de tocarlo. P1 medio (bajó de severidad porque ya no expone PII de contacto, pero sigue siendo un endpoint público sin autenticación de servicio).
...
### ✅ Ya no está pendiente: Sin unique constraint `Veterinarians.UserId`/`Clients.UserId`
Corregido 2026-09-03. Cambios:
- **DB**: `HasIndex(x => x.UserId).IsUnique()` agregado en `VeterinarianConfiguration`/`ClientConfiguration`. Migración generada: `AddClientsAndVeterinariansUserIdUniqueIndex` (`src/Infrastructure/Migrations/20260903142849_...`) — dropea el índice no-único existente en ambas tablas y lo recrea como único. **Sin aplicar todavía contra la base compartida** (`dotnet ef database update`, lo corre el equipo) — igual que pasó con `AddUserAccountsMailUniqueIndex`, **hay que revisar antes que no existan ya filas duplicadas de `UserId`** en `CLIENTS`/`VETERINARIANS` en los datos actuales, o la migración va a fallar al crear el índice único.
- **Aplicación**: `IVeterinarianRepository`/`IClientRepository` ganaron `ExistsByUserIdAsync(userId, ct, excludedId=null)` (mismo patrón que `ExistsByLicenseNumberAsync`/`ExistsByIdentificationNumberAsync`, con exclusión de la propia fila para los `Update`). `CreateVeterinarianCommandValidator`/`UpdateVeterinarianCommandValidator` ganaron una regla `MustAsync` sobre `UserId` (mismo patrón que ya tenían para `LicenseNumber`); `CreateClientCommandHandler`/`UpdateClientCommandHandler` ganaron el mismo chequeo inline (mismo patrón que ya tenían para `IdentificationNumber`) — los cuatro devuelven `ConflictException` (409) con mensaje claro en vez del 409 genérico que hubiera dado la violación de índice único.
- 8 tests nuevos (`CreateVeterinarianCommandValidatorTests`, `UpdateVeterinarianCommandValidatorTests`, `CreateClientCommandHandlerTests`, `UpdateClientCommandHandlerTests` — ninguno de estos cuatro tenía cobertura previa). `dotnet test`: 693/693.
