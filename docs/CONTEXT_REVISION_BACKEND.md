...
| `UserCredentialsController` | POST→`Usuarios:Create`, GET/{id}/by-account→`...View`, PATCH change-password→`Usuarios:Edit` | 🟡 Ver SEC-02 (§4) — pendiente de tu decisión ya tomada (SuperAdmin exclusivo para reset ajeno) |
...
| `AuthController` | register/login/refresh (anónimo), `GET me` (self), **`GET permissions`** (self, nuevo hoy), revoke | No es un módulo CRUD, es infraestructura de auth |
...
Todo lo anterior está en `develop`, comiteado y con `dotnet test` en verde (401/401).
...
### SEC-02 — decisión ya tomada por la líder, falta implementar
`PATCH /api/usercredentials/{id}/change-password` sigue sin validar que el `id` pertenezca al usuario autenticado (usa `RequirePermission("Usuarios", Edit)`, que hoy solo tiene Administrador). Decisión ya registrada en la conversación con la líder: **reset de contraseña ajena (incluyendo la de otro Administrador) queda exclusivo de SuperAdmin** — el Administrador normal pierde el acceso a este endpoint. Falta: (1) mover este endpoint a `SuperAdminOnly`, (2) crear `PATCH /api/auth/me/password` como autoservicio para cualquier rol.
